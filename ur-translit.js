  ...
  function transliterateChars(word){
    ...
    while(i < n){
      ...
      if(INDEP_VOWELS[ch]){
        ...
        if((ch === '\u090f' || ch === '\u0910') &&
           (word[i+1] === '\u0902' || word[i+1] === '\u0901')){
          out += (afterAin ? '' : (prevVowelEnd ? '\u0626' : '')) + '\u06cc\u06ba';
          i += 2; afterAin = false; prevVowelEnd = false; continue;
        }
        if(afterAin){
          out += AIN_VOWEL_MAP[ch];
        }else{
          out += (prevVowelEnd && HIATUS_VOWELS.hasOwnProperty(ch)) ? HIATUS_VOWELS[ch] : INDEP_VOWELS[ch];
        }
        i++; afterAin = false; prevVowelEnd = true; continue;
      }
      afterAin = false;
      /* अनुस्वार (ं) before another consonant is a homorganic nasal STOP
         (अंदर, मंज़िल, संग, ज़िंदा — all dictionary entries above confirm
         this), so it's a plain ن — the ں (noon-ghunna) reading is only for
         word-final nasalisation. चंद्रबिंदु (ँ) always marks vowel
         nasalisation, so it stays ں regardless of what follows. */
      if(ch === '\u0902' && CONSONANTS[word[i+1]]){
