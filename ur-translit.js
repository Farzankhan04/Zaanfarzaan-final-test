...
(function(){
  ...
  /* इज़ाफ़त (اضافت) — "-ए-" joining two words, e.g. राह-ए-ख़ुदा — is NOT
     ...
     Exception: when the FIRST word already ends in an open vowel letter
     (alif ا or waw و), there's no consonant for a bare zer to sit
     under, so izafat bridges it with hamza+ye instead — दवा-ए-दर्द ->
     دوائے درد (not دواِ درد), ख़ू-ए-फ़रामोशी -> خوئے فراموشی. */
  var ZER = '\u0650';
  var HAMZA_YE = '\u0626\u06d2';
    ...
    return text.replace(
      ...
      function(full, firstWord, chain){
        ...
        while((m = linkRe.exec(chain))){
          ...
          if(connector === 'ए'){
            var lastCh = out.charAt(out.length - 1);
            var needsHamza = (lastCh === '\u0627' || lastCh === '\u0648');
            out += (needsHamza ? HAMZA_YE : ZER) + ' ' + nextWord;
          }else{
            out += ' \u0648 ' + nextWord;
          }
        }
  ...
  /* A few Persian/Urdu loan compounds are typed with a joining hyphen in
     the Devanagari source but must render as ONE fused Urdu word, not
     two words split apart by the general hyphen-to-space rule below
     (ख़ुश-बू "khushboo" is one word, خوشبو, not خوش بو). Resolved by
     direct substitution before the general per-word pass ever splits
     on the hyphen. */
  var FUSED_COMPOUNDS_RAW = {'खुश-बू':'خوشبو'};
  var FUSED_COMPOUNDS = {};
  for(var _fk in FUSED_COMPOUNDS_RAW){
    if(FUSED_COMPOUNDS_RAW.hasOwnProperty(_fk)) FUSED_COMPOUNDS[_fk.normalize('NFD')] = FUSED_COMPOUNDS_RAW[_fk];
  }
  function transliterateFusedCompounds(text){
    for(var key in FUSED_COMPOUNDS){
      if(FUSED_COMPOUNDS.hasOwnProperty(key)) text = text.split(key).join(FUSED_COMPOUNDS[key]);
    }
    return text;
  }

  /* The pen name (ज़ान, takhallus in a ghazal's maqta sher) sometimes
     appears wrapped in straight double quotes in the Devanagari source.
     The apostrophe-wrapped form ('ज़ान') already renders quote-free via
     its own dictionary entry above, but a straight double quote isn't
     part of the Devanagari-word regex below, so it would otherwise pass
     straight through into the Urdu output. Strip it here so both
     quoting styles resolve the same way: plain زان, no quote marks. */
  var PEN_NAME_BARE = 'ज़ान'.normalize('NFD');
  var PEN_NAME_DQ_WRAPPED = '"' + PEN_NAME_BARE + '"';
  function stripPenNameQuotes(text){
    return text.split(PEN_NAME_DQ_WRAPPED).join(PEN_NAME_BARE);
  }
  ...
  function transliterateLine(text){
    ...
    text = stripPenNameQuotes(text);
    text = transliterateFusedCompounds(text);
